...
const UserGameDetails_1 = require("../src/models/UserGameDetails");
const Wallet_1 = require("../src/models/Wallet");
// Database connection
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/gatekeepers?directConnection=true&serverSelectionTimeoutMS=2000&appName=gatekeepers';
...
async function clearDatabase() {
    try {
        ...
        await UserGameDetails_1.UserGameDetails.deleteMany({});
        await Wallet_1.Wallet.deleteMany({});
        await OTP_1.OTP.deleteMany({});
...
async function seedUsers() {
    try {
        ...
        const users = await User_1.User.insertMany(sampleUsers);
...
async function seedGames(users) {
    try {
        ...
        const games = await Game_1.Game.insertMany(sampleGames);
...
async function seedUserGameDetails(users, games) {
    try {
        console.log('🔄 Starting to seed user game details...');
        const userGameDetailsData = [
            {
        ...
        const userGameDetails = await UserGameDetails_1.UserGameDetails.insertMany(userGameDetailsData);
...
async function seedWallets(users) {
    try {
        ...
        const wallets = await Wallet_1.Wallet.insertMany(walletData);
...
async function seedOTPs() {
    try {
        ...
        const otps = await OTP_1.OTP.insertMany(otpData);
...
async function updateUserGameDetailsReferences(users, userGameDetails) {
    try {
        console.log('🔄 Starting to update user-game details references...');
        // Update users with their game details references
        for (let i = 0; i < users.length; i++) {
            const updatedUser = await User_1.User.findByIdAndUpdate(users[i]._id, { userGameDetails: [userGameDetails[i]._id] }, { new: true });
...
async function verifySeeding() {
    try {
        ...
        const gameCount = await Game_1.Game.countDocuments();
        const userGameDetailsCount = await UserGameDetails_1.UserGameDetails.countDocuments();
        const walletCount = await Wallet_1.Wallet.countDocuments();
        const otpCount = await OTP_1.OTP.countDocuments();
        ...
        const sampleUser = await User_1.User.findOne({});
        console.log('🧪 Sample user:', {
            name: sampleUser?.name,
            email: sampleUser?.email,
            profileImage: sampleUser?.profileImage
        });
        const sampleGame = await Game_1.Game.findOne({});
...
async function seedDatabase() {
    try {
        ...
        console.log(`🌐 Using database: ${mongoose_1.default.connection.name}`);
        // Clear existing data
        await clearDatabase();
